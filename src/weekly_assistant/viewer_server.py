...
<body>
  ...
  <button id="backTopBtn" class="back-top" type="button" aria-label="Наверх" title="Наверх">
...
.back-top {
  ...
  border-color: #b8c7dd;
  background: #fff;
  box-shadow: 0 10px 26px rgba(31, 35, 40, .16);
  opacity: 0;
  pointer-events: none;
...
@media (max-width: 980px) {
  .shell { width: min(100% - 20px, 760px); padding: 12px 0 96px; }
  ...
  .summary-panel { position: static; max-height: none; overflow: visible; }
  .attention-list { overflow: visible; max-height: none; padding-right: 0; }
...
@media (max-width: 520px) {
  ...
  .actions button { width: 100%; padding: 0 8px; }
  .metric { min-height: 92px; }
  .back-top { right: 12px; bottom: 12px; min-height: 38px; padding: 0 11px; }
  .toast { left: 12px; right: 12px; bottom: 66px; }
}
"""
